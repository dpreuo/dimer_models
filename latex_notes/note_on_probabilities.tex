...
\begin{align}
    K = \sum_{\textbf q}\ketbra{\textbf q}{\textbf q} \otimes \left (
        K_{\bm\Delta} 
        ...
        \right )
\end{align}
where the momentum is constrained to values of 
\begin{align}
    \textbf q = 2\pi \left ( \frac{n_x}{N_x}, \frac{n_y}{N_y} \right ).
\end{align}
Fine, very boring. But now let's consider the fact that on the torus we have to calculate not one but \textit{four} Kasteleyn matrices, where each differ by a change in the boundary conditions! Each sector of $K^{ab}$ has either periodic or anti-periodic boundaries in the $x$- and $y$-directions, which we can accommodate for relatively easily since such a boundary condition only has the effect of modifying which values of $\textbf q$ fit the system. For $K^{ab}$, with $a,b\in \{0,1\}$ we modify the momentum values to,
\begin{align}
    n_x \rightarrow n_x + \frac a2, \qquad
    n_y  \rightarrow n_y + \frac b2.
\end{align}

Next, let us revisit the formula for the overall number of dimersations, \cref{eqn:pbc_n_dimerisations}. The reason for the form of this guy is because each Pfaffian of an individual $K^{ab}$ counts the dimerisations in different sectors with different signs, specifically according to the following table
\begin{center}
...
\begin{align}
    ...
    N_{ox} =N^{oe} + N^{oo} =\frac 12 \left ( - \pf K^{00} + \pf K^{10}\right ) 
\end{align}
